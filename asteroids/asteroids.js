...
class Asteroids {
    ...
    reset() {
        ...
        for (let i = 0; i < num; i++) {
            ...
            while (tryNum++ < maxTries) {
                ...
                const aEnergy = Math.pow(aSize, 2) / 5;
    ...
    animate(a) {
        ...
        a.ship.animate();
        a.ship.shots.forEach((s) => s.animate());
        a.drops.forEach((d) => d.animate());
        // test for crash
        let crashed = false;
    ...
    updateUI(drawShip = true) {
        ...
        // shots
        this.ship.shots.forEach((s) => this.drawObject(s));
        // ship energy, time
 ...
 */
class SpaceObject {
    constructor(game, position, orientation, velocity, size, energy) {
        this.game = game;
     ...
     * @param point point
     */
    circlePolygonHit(center, radius, polygon) {
        // use SAT library
        const circle = new SAT.Circle(new SAT.Vector(center.x, center.y), radius);
        const test = SAT.testPolygonCircle(this.createSatPolygon(polygon), circle);
        return test;
    }
    /**
     ...
     * @param polygon2
     */
    polygonPolygonHit(polygon1, polygon2) {
        // use SAT library
        const test = SAT.testPolygonPolygon(this.createSatPolygon(polygon1), this.createSatPolygon(polygon2));
        return test;
    }
    /**
     * Convert Array<Vector2D> to SAT.Polygon
     */
    createSatPolygon(polygon) {
        ...
        const poly = new SAT.Polygon(position, polPoints);
        return poly;
    }
}
...
class Spaceship extends SpaceObject {
    constructor(game, position, orientation, velocity, size, energy, power) {
    ...
    shoot() {
        ...
        const velocity = this.getOrientationVector()
            ...
            .multiplyFactor(1);
        const shot = new Shot(this.game, position, this.orientation, velocity, 5, this.power);
...
class Shot extends SpaceObject {
    constructor(game, position, orientation, velocity, size, energy) {
        ...
        const p = position;
        this.points = [
            p.clone().add(this.getOrientationVector()
                .multiplyFactor(this.size)),
            p.clone().add(this.getOrientationVector()
...
class Asteroid extends SpaceObject {
    ...
    split() {
        ...
        for (let i = 0; i < numberChildren; i++) {
            ...
            const child = new Asteroid(this.game, this.position.clone().add(Vector2D.randomVector(-1, 1, -1, 1)), this.orientation, this.velocity.clone().add(Vector2D.randomVector(-1, 1, -1, 1)), size, energy);
            children.push(child);
        }
