...
class SpaceObject
{
    ...
    public draw(ctx: CanvasRenderingContext2D): void
    {
        lib.drawCircle(
            ...
            this.position,
            this.size,
            SpaceObject.strokeStyle,
            SpaceObject.fillStyle
        );
    }
    ...
    {
        // draw orientation
        const point = this.position.clone().add(
            this.getOrientationVector().multiplyFactor(this.size * 2)
        );
        ctx.beginPath();
        ...
        ctx.fillStyle = "#fff";
        ctx.fillText(
            (Math.floor(this.energy)).toString(),
            this.position.x,
            this.position.y
        );
    }
...
class Asteroid extends SpaceObject
{
     ...
     */
    public hitBy(object: SpaceObject): [Drop | null, Array<Asteroid> | null]
    {
